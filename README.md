...
로그인 회원은 사이트맵에서 추천한 핵심 페이지 2~5개를 묶어 비교하고, 검사 이력 두 시점의 전후 보고서를 링크나 PDF로 공유할 수 있습니다. 브랜드명이나 주소를 Gemini에 알려주지 않은 소비자 질문으로 모델이 가게를 기억하는지도 확인합니다. 운영자 인터뷰 답변과 생성 초안은 공용 검사 캐시에 저장하지 않습니다.
...
- [전체 기능·보안·운영 한도·검증 결과](docs/FEATURES.md)

## 핵심 기능

| 영역 | 제공 기능 |
|---|---|
| 진단 | 공개 페이지의 AI 정보·고객 정보 2축 점수, 15개 세부 항목, 근거와 우선 수정 3개 |
| AI 지원 | Gemini 브랜드 설명, 소개 초안 3개, 고객 질문 5개, 운영자 인터뷰와 FAQ, 브랜드 비노출 검사 |
| 개선 | 코드 예시, 플랫폼별 수정 안내, 체크리스트, 재검사와 전후 비교 |
| 회원 | 이메일 인증, Google·카카오·네이버 로그인, 사이트 저장, 90일 이력, 비밀번호 변경과 탈퇴 |
| 사이트 전체 | 사이트맵 핵심 페이지 추천, 2~5개 묶음 검사 |
| 공유 | 단일·전후 비교 보고서의 30일 읽기 전용 링크, 인쇄·PDF 저장 |
| 접근 제한 대응 | 로그인·성인인증 후 사용자가 직접 현재 탭 정보를 보내는 Chrome 확장프로그램 |

2026-09-16 기준 자동 테스트 56개, 운영 통합 점검 25개, 프로덕션 빌드와 의존성 보안 검사를 통과했습니다. 검증 범위와 자동으로 확인할 수 없는 외부 로그인·메일 수신 항목은 [기능과 검증 상태](docs/FEATURES.md)에 구분해 기록했습니다.
...
Chrome의 `chrome://extensions`에서 개발자 모드를 켜고 `extension/` 폴더를 압축해제된 확장프로그램으로 로드하세요.
...
운영자 개선 사례를 수집할 때는 [사례 수집 양식](docs/CASE_STUDY_TEMPLATE.md)을 사용합니다. 소유권 확인과 게시 동의가 끝난 사례만 공개 목록에 등록합니다.
