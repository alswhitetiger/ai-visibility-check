...
async function getJson(url, signal) {
  ...
  if (!r.ok || d.error) throw Object.assign(new Error(d.message || ({ INVALID_URL: '공개된 쇼핑몰 주소를 입력해 주세요.', FETCH_FAILED: '페이지를 읽지 못했습니다. 주소를 확인하거나 다른 페이지를 검사해 주세요.' }[d.error]) || '지금은 이 페이지를 검사할 수 없습니다. 잠시 후 다시 시도해 주세요.'), { code: d.error, usage: d.usage });
  return d;
}
...
function AiAnswer({ ai, example }) {
  ...
  return <details className="panel ai-panel"><summary>추가 확인 · AI에게 이 브랜드를 물어봤어요</summary>
    ...
    {example ? <p>이 예시에는 실제 AI 응답을 넣지 않았습니다.</p> : !answer ? <p>{aiFailureHelp(ai)} 위 페이지 검사 결과는 확인할 수 있습니다. AI 답변은 점수에 포함하지 않습니다.</p> : <>
...
function Report({ data, previous, onScan, onExample, user }) {
  ...
  return <main id="report" tabIndex={-1}>
    ...
    {!data.example && !blocked && !data.ai?.answer && <div className="notice"><b>페이지 검사 완료 · AI 답변 미수집</b><p>{aiFailureHelp(data.ai)}</p><p>페이지 점수와 수정 안내는 이용할 수 있어요. AI 답변을 위해 반복 재검사할 필요는 없습니다.</p></div>}
...
export default function Experience() {
  ...
  return <div className="app-shell">
    ...
    {state.status === 'error' && <div className="notice" role="alert"><h3>{failureHelp(state.code).title}</h3><p>{state.message}</p><p>{failureHelp(state.code).next}</p>{state.code === 'LOGIN_WALL' && <div className="panel"><h3>고객이 직접 확인할 항목</h3><p>검사 완료나 점수에 반영되는 항목은 아닙니다. 비밀번호·로그인 쿠키를 이 서비스에 제출하지 마세요.</p>{/^https?:\/\//i.test(state.target || '') && <a href={state.target} target="_blank" rel="noreferrer">해당 사이트에서 직접 확인하기 ↗</a>}<ol><li>쇼핑몰에서 직접 로그인·성인인증을 완료하세요.</li><li>해당 페이지의 제목, 설명, 가격과 이미지 안내가 실제로 표시되는지 확인하세요.</li><li>휴대폰에서 글과 버튼을 읽고 사용할 수 있는지 확인하세요.</li></ol><ScreenshotGuide url={state.target} /></div>}{state.target && failureHelp(state.code).retry && <button className="button secondary" onClick={() => scan(state.target)}>다시 시도</button>}<button className="text-button" onClick={() => example()}>예시 결과 보기 →</button>{latest.current && <button className="text-button" onClick={() => setState({status:'done',data:latest.current})}>직전 결과 다시 보기</button>}</div>}
