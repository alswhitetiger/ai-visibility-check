...
export default function Experience() {
  ...
  return <div className="app-shell">
    ...
    {state.status === 'error' && <div className="notice" role="alert"><h3>{failureHelp(state.code).title}</h3><p>{state.message}</p><p>{failureHelp(state.code).next}</p>{state.target && failureHelp(state.code).retry && <button className="button secondary" onClick={() => scan(state.target)}>다시 시도</button>}<button className="text-button" onClick={() => example()}>예시 결과 보기 →</button>{latest.current && <button className="text-button" onClick={() => setState({status:'done',data:latest.current})}>직전 결과 다시 보기</button>}</div>}
