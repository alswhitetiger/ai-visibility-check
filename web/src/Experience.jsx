...
function Report({ data, previous, onScan, onExample, user }) {
  ...
  return <main id="report" tabIndex={-1}>
    ...
    {blocked ? <div className="notice"><p>{data.skipReason === 'robots_unavailable' ? '사이트의 접근 규칙(robots.txt)을 읽지 못해 검사를 멈췄습니다. 허용 여부를 모르는 상태를 통과로 처리하지 않습니다.' : '사이트가 게시한 접근 제한에 따라 페이지를 가져오지 않았습니다.'}</p><p>다음 할 일: 운영 담당자에게 해당 주소의 접근 규칙과 robots.txt 응답을 확인해 달라고 요청하세요. 접근 상태를 확인한 뒤 다시 검사하세요.</p>{data.robots && <p>게시된 규칙상 AI {data.robots.answerTotal}종 중 {data.robots.answerAllowed}종은 입력한 주소에 접근 허용으로 표시됩니다. 실제 접근 성공 여부는 별개입니다.</p>}</div> : <>
      ...
      {!data.shared && !data.example && user && <SiteBundleAudit key={'bundle:'+data.url} data={data} />}
      <ObservedInfo data={data} />
      {!data.shared && !data.example && <AiDraft key={'draft:' + data.url + ':' + data.scannedAt} data={data} />}
      {!data.shared && !data.example && <CustomerQuestions key={'questions:' + data.url + ':' + data.scannedAt} data={data} />}
      {!data.shared && !data.example && <OperatorInterview key={'interview:' + data.url + ':' + data.scannedAt} data={data} />}
      {!data.shared && !data.example && user && <DiscoveryTest key={'discovery:' + data.url} data={data} />}
