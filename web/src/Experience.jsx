...
export default function Experience() {
  ...
  return <div className="app-shell">
    ...
    {state.status === 'error' && <div className="notice" role="alert"><h3>{failureHelp(state.code).title}</h3><p>{state.message}</p><p>{failureHelp(state.code).next}</p>{state.code === 'LOGIN_WALL' && <div className="panel"><h3>고객이 직접 확인할 항목</h3><p>검사 완료나 점수에 반영되는 항목은 아닙니다. 비밀번호·로그인 쿠키를 이 서비스에 제출하지 마세요.</p>{/^https?:\/\//i.test(state.target || '') && <a href={state.target} target="_blank" rel="noreferrer">해당 사이트에서 직접 확인하기 ↗</a>}<ol><li>쇼핑몰에서 직접 로그인·성인인증을 완료하세요.</li><li>해당 페이지의 제목, 설명, 가격과 이미지 안내가 실제로 표시되는지 확인하세요.</li><li>휴대폰에서 글과 버튼을 읽고 사용할 수 있는지 확인하세요.</li></ol></div>}{state.target && failureHelp(state.code).retry && <button className="button secondary" onClick={() => scan(state.target)}>다시 시도</button>}<button className="text-button" onClick={() => example()}>예시 결과 보기 →</button>{latest.current && <button className="text-button" onClick={() => setState({status:'done',data:latest.current})}>직전 결과 다시 보기</button>}</div>}
