export function failureHelp(code) {
  const cases = {
    ...
    ANON_LIMIT: { title: '오늘의 무료 체험을 사용했어요', next: '로그인하면 계정 기록과 추가 검사를 이용할 수 있습니다. 예시 결과는 계속 볼 수 있어요.' },
    ...
    LOGIN_WALL: { title: '로그인 뒤의 내용은 확인할 수 없어요', next: '쇼핑몰 로그인·성인인증은 해당 사이트에서 직접 진행하세요. 브라우저 로그인 상태는 검사 서버에 전달되지 않으므로, 로그인 후 재검사해도 같은 화면이 나올 수 있어요. 공개 페이지 주소로 검사하거나 화면 캡처를 운영 담당자에게 전달해 수동 확인하세요.' },
