...
- 2026-09-14 설정 진행: 카카오 앱 `1576595`(가게 체크)에 운영 Callback URL과 대표 도메인을 등록했고, 닉네임은 선택 동의로 설정했습니다. 이메일은 아직 `권한 없음` 상태이며, 비즈 앱 전환 전에 앱 아이콘 등록이 필요합니다.
- Google 전용 프로젝트 `store-check-508603`(Store Check)를 만들었습니다. 앱 이름·지원 이메일·외부 사용자 설정을 입력했고, 사용자 데이터 정책 동의 단계에서 대기 중입니다. OAuth 클라이언트는 아직 발급하지 않았습니다.
- 위 내용은 설정 진행 상황이며 로그인 성공을 뜻하지 않습니다. 서버 secret 등록과 실제 로그인 검증이 모두 끝나야 사용 가능합니다.
...
3. 동의항목에서 최소한 카카오계정 이메일과 닉네임을 설정합니다. 이메일이 없으면 이 서비스의 계정 식별을 완료할 수 없습니다.
   - 이메일이 `권한 없음`이면 앱 기본 정보에 아이콘을 등록하고 비즈 앱 전환 절차를 진행합니다. 개인 개발자는 본인인증과 카카오비즈니스 약관 동의가 필요합니다.
   - 서버는 이메일·닉네임만 요청합니다. 프로필 사진 권한은 요청하지 않습니다.
