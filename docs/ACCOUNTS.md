...
현재 운영에서는 사용자가 이전 프로젝트에서 검증한 개인 Gmail SMTP를 사용한다. Google 계정의 2단계 인증과 앱 비밀번호를 준비한 뒤 다음 값을 Worker secret으로 설정한다.
...
- 이메일 인증번호 로직은 모의 메일 응답으로 발송 내용·해시 저장·인증 전 세션 차단·인증 후 세션 발급을 자동 검증했다. 운영 Gmail SMTP 연결 뒤 실제 받은 편지함 검증을 진행한다.
