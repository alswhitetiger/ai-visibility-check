...
운영에서는 Resend의 발신 도메인을 인증한 뒤 다음 값을 설정한다.

- `RESEND_API_KEY`: Worker secret
- `AUTH_EMAIL_FROM`: 인증된 발신 주소 (Worker 변수 또는 secret)

두 값이 모두 있으면 인증번호 회원가입과 비밀번호 재설정 메일 기능이 켜진다. 설정 뒤 실제 받은 편지함에서 가입·재발급·오입력 제한·만료·인증 후 로그인까지 검증한다. 기본 계정의 비밀번호 변경은 로그인한 상태에서 기존 비밀번호를 확인한 후 제공한다.

## 검증 상태 및 남은 일

- SQLite/D1 호환 어댑터 테스트: 가입·암호 해시·로그아웃, 데이터 소유권, 외부 Origin 차단, 동시 요청 한도, 자정 초기화, 캐시 무료 조회와 실패 환불.
- 이메일 인증번호 로직은 모의 Resend 응답으로 발송 내용·해시 저장·인증 전 세션 차단·인증 후 세션 발급을 자동 검증했다. 운영 Resend 키와 인증된 발신 주소를 등록한 뒤 실제 받은 편지함 검증이 필요하다.
