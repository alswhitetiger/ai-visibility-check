...
이메일 회원가입은 이름·이메일·12자 이상 비밀번호와 필수 개인정보 수집·이용 동의를 받은 뒤 6자리 인증번호를 발송한다. 인증번호는 10분 동안 유효하고 5회 잘못 입력하면 폐기된다. DB에는 번호 원문 대신 해시를 저장하며, 인증을 마치기 전에는 로그인 세션을 발급하지 않는다. 메일 설정이 없으면 화면과 API 모두 이메일 가입을 차단해 미인증 계정이 생기지 않게 한다.

가입 화면에는 수집 목적, 필수·자동 생성 항목, 보유 기간, 동의 거부 권리와 이용 제한을 표시한다. 아이디 찾기는 사용자가 입력한 주소가 인증된 가입 이메일일 때만 확인 메일을 보내고, 화면에는 계정 존재 여부와 관계없이 같은 결과를 표시한다. 계정 화면의 회원 탈퇴는 현재 비밀번호 또는 최근 소셜 로그인 세션으로 본인을 확인한 뒤 회원 관련 데이터를 영구 삭제한다. 전체 고지문은 `web/public/privacy-consent.md`에서 관리하며 배포 후 `/ai-visibility-check/privacy-consent.md`로 확인할 수 있다.
...
- `AUTH_EMAIL_FROM`: 메일에 표시할 발신 주소

`GMAIL_SMTP_USER`와 `GMAIL_APP_PASSWORD`가 모두 있으면 인증번호 회원가입과 비밀번호 재설정 메일 기능이 켜진다. Worker는 `smtp.gmail.com:465`에 TLS로 연결한다. 설정 뒤 실제 받은 편지함에서 가입·재발급·오입력 제한·만료·인증 후 로그인까지 검증한다. 기본 계정의 비밀번호 변경은 로그인한 상태에서 기존 비밀번호를 확인한 후 제공한다.

## 검증 상태 및 남은 일

- SQLite/D1 호환 어댑터 테스트: 가입·암호 해시·로그아웃, 데이터 소유권, 외부 Origin 차단, 동시 요청 한도, 자정 초기화, 캐시 무료 조회와 실패 환불.
- 이메일 인증번호 로직은 모의 메일 응답으로 발송 내용·해시 저장·인증 전 세션 차단·인증 후 세션 발급을 자동 검증했다. 2026-09-14 운영 Gmail SMTP 발송과 Gmail 실제 수신, 네이버 주소에 대한 Gmail 서버 접수까지 확인했다.
- 2026-09-09 Workers 실서버에서 기존 이메일 가입·로그인·보안 쿠키·사이트 저장과 삭제·실제 페이지 검사·이력 저장·한도 20→19 차감·로그아웃을 검증했다. 테스트 계정은 제거했다. 2026-09-16 회원·검사·비교 공유·사이트맵 추천·삭제·AI 입력 분리를 포함한 자동 테스트 56개가 통과했다.
- 2026-09-16 운영 통합 점검 25개에서 세 OAuth 진입점, 확장 API, 임시 회원의 가입·로그인·사이트·검사·Gemini 생성·이력·체크리스트·단일/비교 공유·비밀번호 변경·탈퇴가 통과했다. 임시 회원은 삭제 후 0명임을 D1에서 확인했다.
- Google·카카오·네이버 OAuth 설정과 이메일 인증 발송은 운영 Worker에 연결되어 있다. 각 제공자의 동의항목 변경이나 Gmail 앱 비밀번호 교체 뒤에는 운영 로그인을 다시 검증한다.
- 이력은 90일 범위만 조회 가능하며 새 이력 저장 때 해당 회원의 오래된 이력을 정리한다. 회원 탈퇴 시 계정에 연결된 사이트·이력·이용 기록·개선 체크리스트·공유 보고서도 함께 삭제한다.
