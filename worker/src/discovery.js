...
export function mentionsTarget(result, { host, brand }) {
  const haystack = clean(result.text);
...
export async function discoverBrand(env, { query, host, brand }) {
  const result = await queryGemini(env, query);
  if (!result.ok) return { error: 'AI_UNAVAILABLE', message: 'Gemini 답변을 받지 못했습니다. 잠시 후 다시 시도해 주세요.', kind: result.kind };
  return { ...result, found: mentionsTarget(result, { host, brand }), targetHost: host };
}
