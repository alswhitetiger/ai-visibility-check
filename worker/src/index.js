  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (pathname === '/api/health') {
        return json({ ok: true, version: DIAGNOSIS_VERSION, ts: Date.now(), anonymousScan: env.ANON_SCAN_ENABLED === 'true', aiProviders: { gemini: !!env.GEMINI_API_KEY }, features: { brandBlindDiscovery: !!env.GEMINI_API_KEY, sitemapSuggestions: true, comparisonSharing: true }, cache: { enabled: !!env.DB, ttlHours: Number(env.CACHE_TTL_HOURS || 24) } }, 200, origin);
      }
